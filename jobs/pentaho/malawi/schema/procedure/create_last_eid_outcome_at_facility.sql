...
BEGIN
    ...
    CREATE TEMPORARY TABLE last_eid_facility_outcome(
	 ...
	 INSERT INTO last_eid_facility_outcome (index_desc, pat,identifier, state,gender, birthdate,program,program_state_id, start_date,end_date,location)
    select index_desc, pat,identifier, state,gender, birthdate,program,program_state_id, start_date,end_date,location
from (
	SELECT
index_desc,
...
FROM (SELECT
      ...
      FROM omrs_program_state,
            ...
            join mw_patient mw on index_descending.patient_id = mw.patient_id
